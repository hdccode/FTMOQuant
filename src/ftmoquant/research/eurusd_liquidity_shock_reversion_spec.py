"""Strict preregistration loader for ``eurusd_liquidity_shock_reversion_v1``."""
...
EURUSD_LIQUIDITY_SHOCK_REVERSION_SPEC_PATH = Path(
...
class EurusdLiquidityShockReversionSpecError(ValueError):
    """Raised when the preregistration is ambiguous, incomplete, or changed."""


@dataclass(frozen=True, slots=True)
class EurusdLiquidityShockReversionSpec:
    family_id: str
    version: str
    economic_hypothesis: str
    baseline_prior_returns_grid: tuple[int, ...]
    shock_multiple_grid: tuple[float, ...]
    hold_eligible_minutes_grid: tuple[int, ...]
    expected_unique_trial_count: int
    semantic_sha256: str
    canonical_document: dict[str, Any]

    @property
    def shock_multiple_decimals(self) -> tuple[Decimal, ...]:
        return tuple(Decimal(str(value)) for value in self.shock_multiple_grid)
...
def load_eurusd_liquidity_shock_reversion_spec(
    path: Path = EURUSD_LIQUIDITY_SHOCK_REVERSION_SPEC_PATH,
) -> EurusdLiquidityShockReversionSpec:
    """Load and prove the exact pre-return 36-cell family protocol."""

    try:
        value = yaml.safe_load(path.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as error:
        raise EurusdLiquidityShockReversionSpecError(
    ...
    if semantic != EURUSD_LIQUIDITY_SHOCK_REVERSION_SEMANTIC_SHA256:
        raise EurusdLiquidityShockReversionSpecError(
            "eurusd_liquidity_shock_reversion_v1 preregistration semantic SHA drifted"
        )
    ...
    if (
        ...
        or family["family_id"] != "eurusd_liquidity_shock_reversion_v1"
    ...
    if session != {"session_id": "all", "optimized": False}:
        raise EurusdLiquidityShockReversionSpecError("session must be frozen to All")

    parameter_grid = _mapping(document["parameter_grid"], "parameter_grid")
    _exact_keys(
        parameter_grid,
        {
            "baseline_prior_returns",
            "shock_multiple",
            "hold_eligible_minutes",
            "combination_rule",
    ...
    baseline_grid = tuple(
        _integer(item, "baseline_prior_returns")
        for item in _list(parameter_grid["baseline_prior_returns"], "baseline grid")
    )
    multiple_grid = tuple(
        _float(item, "shock_multiple")
        for item in _list(parameter_grid["shock_multiple"], "shock multiple grid")
    )
    hold_grid = tuple(
        _integer(item, "hold_eligible_minutes")
        for item in _list(parameter_grid["hold_eligible_minutes"], "hold grid")
    )
    if baseline_grid != (30, 60, 120):
        raise EurusdLiquidityShockReversionSpecError("baseline_prior_returns drifted")
    if multiple_grid != (3.0, 4.0, 5.0, 6.0):
        raise EurusdLiquidityShockReversionSpecError("shock_multiple grid drifted")
    if hold_grid != (5, 15, 30):
        raise EurusdLiquidityShockReversionSpecError("hold_eligible_minutes drifted")
    if parameter_grid["combination_rule"] != "full_unconditional_cartesian_product":
        raise EurusdLiquidityShockReversionSpecError("combination rule drifted")
    expected_count = _integer(
        parameter_grid["expected_unique_trial_count"], "expected trial count"
    )
    actual_count = len(baseline_grid) * len(multiple_grid) * len(hold_grid)
    if expected_count != 36 or actual_count != expected_count:
        raise EurusdLiquidityShockReversionSpecError(
            "exact grid must contain exactly 36 cells"
        )
    ...
    return EurusdLiquidityShockReversionSpec(
        family_id="eurusd_liquidity_shock_reversion_v1",
        version="1.0.0",
        economic_hypothesis=hypothesis,
        baseline_prior_returns_grid=baseline_grid,
        shock_multiple_grid=multiple_grid,
        hold_eligible_minutes_grid=hold_grid,
...
def _validate_signal(document: dict[str, Any]) -> None:
    ...
    if (
        ...
        or signal.get("return_formula") != "ln(C_t / C_t_minus_1)"
        or signal.get("eligible_return")
        != "consecutive_observed_valid_completed_1m_midpoint_closes"
        or signal.get("missing_or_invalid_rule")
        != "no_signal_and_not_an_eligible_return_reset_chain"
        or signal.get("fill_or_interpolation") is not False
        or signal.get("baseline_scale") != "median_absolute_log_return"
        or signal.get("baseline_observation_window") != "baseline_prior_returns"
        or signal.get("shock_rule") != "abs(r_t) > shock_multiple * baseline_scale"
        or signal.get("positive_shock_target") != -1
        or signal.get("negative_shock_target") != 1
        or signal.get("zero_or_invalid_scale") != "no_signal"
        or signal.get("insufficient_history") != "no_signal"
        or signal.get("signals_while_position_or_pending") != "ignore"
        or signal.get("raw_magnitudes") != [-1, 0, 1]
    ):
        raise EurusdLiquidityShockReversionSpecError("signal semantics drifted")
...
def _validate_risk_execution(document: dict[str, Any]) -> None:
    ...
    if (
        ...
        or execution.get("hold_eligible_completed_minutes_after_entry")
        != "hold_eligible_minutes"
        or execution.get("max_open_positions") != 1
        or execution.get("overlapping_shocks_while_positioned_or_pending")
        != "ignored"
...
def _validate_folds(document: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise EurusdLiquidityShockReversionSpecError(
            "DEVELOPMENT fold identity drifted"
        )
    expected = [
        {
    ...
    if declared.get("folds") != expected:
        raise EurusdLiquidityShockReversionSpecError("declared fold timestamps drifted")
...
def _validate_selection_and_seals(document: dict[str, Any]) -> None:
    ...
    if (
        ...
        != ["baseline_prior_returns", "shock_multiple", "hold_eligible_minutes"]
...
def _validate_evaluator_sample_metrics(document: dict[str, Any]) -> None:
    ...
    if (
        sample.get("fold_metrics_trade_count") != "completed_executed_shock_round_trips"
        or sample.get("definition")
        != "one_entry_plus_its_eventual_exit_equals_one_evidential_trade"
        or sample.get("count_entry_and_exit_separately") is not False
        or sample.get("count_ignored_shocks_while_positioned") is not False
        ...
        != "eurusd-liquidity-shock-reversion-v1-development-evaluator-1"
        ...
        or metrics.get("trade_count") != "completed_executed_shock_round_trips"
...
def _list(value: object, description: str) -> list[Any]:
    if not isinstance(value, list):
        raise EurusdLiquidityShockReversionSpecError(f"{description} must be a list")
    return value
...
def _float(value: object, description: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise EurusdLiquidityShockReversionSpecError(f"{description} must be numeric")
    return float(value)
