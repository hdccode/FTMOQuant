"""Production DEVELOPMENT evaluator and exact-grid runner for
``eurusd_liquidity_shock_reversion_v1``.
...
Reuses the generic G1 engine (:mod:`ftmoquant.research.g1.search`,
:mod:`ftmoquant.research.g1.selector`, :mod:`ftmoquant.research.g1.artifacts`)
and the genuinely family-agnostic execution/statistics helpers already proven
by ``eurusd_tsm_development.py`` (scaled-instruction shape, native fold
runner primitives, fold-metric aggregation) instead of duplicating them.
"""
...
from ftmoquant.research.eurusd_liquidity_shock_reversion_spec import (
    ...
    EurusdLiquidityShockReversionSpec,
...
EVALUATOR_VERSION = "eurusd-liquidity-shock-reversion-v1-development-evaluator-1"
RESULT_SCHEMA = "ftmoquant.g1-eurusd-liquidity-shock-reversion-development-results"
BASE_RESEARCH_UNITS = Decimal("100000")
_EURUSD = "EUR/USD.DUKASCOPY"
_SESSION_ZONE = ZoneInfo("America/New_York")


class EurusdLiquidityShockReversionEvaluationError(ValueError):
    """Raised when the frozen production evaluation cannot be honored exactly."""


class PreparedEurusdLiquidityShockMarketData:
    """One verified DEVELOPMENT load, sliced in-memory for immutable folds."""

    __slots__ = ("instrument", "minute_bars", "execution_frames", "daily_midpoints")
    ...
    def __init__(
        ...
        self.execution_frames = execution_frames
        self.daily_midpoints = daily_midpoints


class EurusdLiquidityShockReversionFamily(
    StrategyFamily[
        Sequence[tuple[int, int, Decimal]],
        tuple[Any, ...],
    ]
):
    """The frozen exact 36-cell ``eurusd_liquidity_shock_reversion_v1`` family."""

    def __init__(self, spec: EurusdLiquidityShockReversionSpec | None = None) -> None:
        self.spec = (
            load_eurusd_liquidity_shock_reversion_spec() if spec is None else spec
        )
        self._parameter_space = ParameterSpace(
            (
                CategoricalParameter(
                    "baseline_prior_returns", self.spec.baseline_prior_returns_grid
                ),
                CategoricalParameter(
                    "shock_multiple", self.spec.shock_multiple_grid
                ),
                CategoricalParameter(
                    "hold_eligible_minutes", self.spec.hold_eligible_minutes_grid
                ),
            )
        )
    ...
    def create_state(
        ...
        return EurusdLiquidityShockReversionState(_parameters(parameters))

    def build_signals(
        self,
        data: Sequence[tuple[int, int, Decimal]],
        parameters: ParameterMapping,
    ) -> tuple[Any, ...]:
        state = self.create_state(parameters)
        return tuple(
            signal
            for event_ns, information_ns, midpoint in data
            if (signal := state.on_minute_close(event_ns, information_ns, midpoint))
            is not None
        )
...
class EurusdLiquidityShockReversionTrialEvaluator:
    ...
    def __init__(
        ...
        spec: EurusdLiquidityShockReversionSpec,
    ...
    def __call__(
        ...
        if not isinstance(family, EurusdLiquidityShockReversionFamily):
            raise EurusdLiquidityShockReversionEvaluationError(
                "evaluator received the wrong family"
            )
        family.validate_parameters(parameters)
        _validate_window(window, self.search_context)
        frames = tuple(
            item
            for item in self.prepared.execution_frames
...
def prepare_eurusd_liquidity_shock_market_data(
    context: DevelopmentResearchContext,
    development_roots: Mapping[str, Path],
) -> PreparedEurusdLiquidityShockMarketData:
    """Load authoritative EUR/USD DEVELOPMENT 1-minute streams exactly once."""
    ...
    if len(found) != 1 or not isinstance(found[0], CurrencyPair):
        raise EurusdLiquidityShockReversionEvaluationError(
            "frozen EUR/USD CurrencyPair is unavailable"
        )
    ...
    execution_frames = _minute_execution_frames(minute)
    daily = _daily_midpoints_from_minute_bars(minute)
    _validate_catalog_trees(context, development_roots)
    return PreparedEurusdLiquidityShockMarketData(
        instrument, minute, execution_frames, daily
    )


def run_eurusd_liquidity_shock_reversion_development(
    *,
    spec_path: Path,
    ...
    """Execute the exact 36-cell DEVELOPMENT grid and persist deterministic output."""

    if output_dir.exists():
        raise FileExistsError(f"output already exists: {output_dir}")
    _require_clean_worktree()
    spec = load_eurusd_liquidity_shock_reversion_spec(spec_path)
    if spec.semantic_sha256 != EURUSD_LIQUIDITY_SHOCK_REVERSION_SEMANTIC_SHA256:
        raise EurusdLiquidityShockReversionEvaluationError(
            "semantic SHA mismatch blocks runner"
        )
    ...
    prepared = prepare_eurusd_liquidity_shock_market_data(context, development_roots)
    ...
    if result.exact_trial_count != 36:
        raise EurusdLiquidityShockReversionEvaluationError(
            "production search did not attempt 36 cells"
        )
...
def _build_scaled_instructions(
    ...
    for event_ns, information_ns, midpoint in frames:
        if information_ns < evaluate_start:
            state.warmup_minute_close(event_ns, information_ns, midpoint)
            continue
        state.on_minute_close(event_ns, information_ns, midpoint)
        executable = state.on_execution_frame(event_ns, information_ns)
        if executable is None or not (
            evaluate_start <= executable.execution_information_ns < evaluate_end
        ):
            continue
        if executable.target is RawDirectionalTarget.FLAT:
            target_units = Decimal(0)
            count_alpha_transition = False
            reason = "hold_period_flatten"
        else:
            decision = normalizer.target_at(
                float(executable.target),
                executable.signal_information_ns,
                estimator,
            )
            target_units = decision.target_base_units(BASE_RESEARCH_UNITS).quantize(
                Decimal("0.00000001")
            )
            count_alpha_transition = True
            reason = "shock_entry"
        if target_units == planned_units:
            continue
        result.append(
            ScaledTargetInstruction(
                execution_event_ns=executable.execution_event_ns,
                execution_information_ns=executable.execution_information_ns,
                decision_information_ns=executable.signal_information_ns,
                raw_target=executable.target,
                target_base_units=target_units,
                execution_midpoint=midpoint,
                count_alpha_transition=count_alpha_transition,
                reason=reason,
            )
        )
    ...
    if planned_units != 0:
        final = next(
            (item for item in reversed(frames) if item[1] < evaluate_end), None
        )
        ...
        result.append(
            ScaledTargetInstruction(
                ...
                raw_target=state.latest_target,
    ...
    if len({item.execution_event_ns for item in result}) != len(result):
        raise EurusdLiquidityShockReversionEvaluationError(
            "multiple targets mapped to one execution frame"
        )
    return tuple(result)


class _LiquidityShockExecutor(Strategy):
    def __new__(
        cls,
        ...
        super().__init__(
            StrategyConfig(
                strategy_id=StrategyId(
                    "EURUSD-LIQUIDITY-SHOCK-REVERSION-V1-DEVELOPMENT-001"
                ),
                log_events=False,
                log_commands=False,
            )
    ...
    def on_bar(self, bar: Bar) -> None:
        ...
        if local.weekday() <= 4 and local.hour == 17 and local.minute == 0:
            self.clock.set_time_alert_ns(
                name=f"eurusd-liquidity-shock-mark-{bar.ts_event}",
...
def _run_native_fold(
    *,
    ...
    identity = _sha256_json(
        {
            "family_semantic_sha256": EURUSD_LIQUIDITY_SHOCK_REVERSION_SEMANTIC_SHA256,
            "fold_id": fold_id,
            "instructions": [asdict(item) for item in instructions],
        }
    )
    engine = BacktestEngine(_engine_config(identity))
    executor = _LiquidityShockExecutor(
    ...
    try:
        ...
        engine.run(
            ...
            run_config_id=f"G1LSR-{identity[:20]}",
        )
...
def _parameters(
    ...
    baseline = parameters.get("baseline_prior_returns")
    multiple = parameters.get("shock_multiple")
    hold = parameters.get("hold_eligible_minutes")
    if (
        isinstance(baseline, bool)
        or not isinstance(baseline, int)
        or not isinstance(multiple, float)
    ...
    return EurusdLiquidityShockReversionParameters(
        ...
        shock_multiple=Decimal(str(multiple)),
...
def _write_result_artifacts(
    ...
    write_runtime_provenance(
        output_dir / "runtime_provenance.json",
        {
            "git_commit": _git_commit(),
            "python_module": (
                "ftmoquant.research.eurusd_liquidity_shock_reversion_development"
            ),
        },
    )
...
def _development_root(value: str) -> tuple[str, Path]:
    ...
    try:
        _reject_sealed_path(path)
    except Exception as error:  # noqa: BLE001 - argparse requires this shape
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Execute frozen eurusd_liquidity_shock_reversion_v1 exact "
            "DEVELOPMENT grid"
        )
    )
    parser.add_argument(
        "--spec", type=Path, default=EURUSD_LIQUIDITY_SHOCK_REVERSION_SPEC_PATH
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if len(roots) != len(pairs):
        raise EurusdLiquidityShockReversionEvaluationError(
            "duplicate development-root instrument"
        )
    _, _, summary = run_eurusd_liquidity_shock_reversion_development(
