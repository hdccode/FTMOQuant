"""B3F3 DEVELOPMENT screen: wires the causal signal
(:mod:`ftmoquant.research.alpha_lab.b3f3_session_open_mr_signals`) and
execution (:mod:`ftmoquant.research.alpha_lab.b3f3_session_open_mr_execution`)
layers into the frozen v2 hard gates and diagnostics, per instrument sleeve
and per statistical config.

Structurally identical to
:mod:`ftmoquant.research.alpha_lab.b3f2_asian_range_fade_screen` (gate
arithmetic, thresholds read programmatically from
:data:`ftmoquant.research.alpha_lab.batch3_preregistration_v2.
DEVELOPMENT_GATES`, N-D parameter-neighborhood robustness reusing
...
_axis_adjacent_neighbors`) -- reimplemented here (not imported) only
because B3F3's row shape carries its own diagnostics (entry-time
distribution, mean/median displacement at entry) not present on B3F2's row.
"""
...
FAMILY_ID = "B3F3_session_open_microstructure_mean_reversion"
...
ARTIFACT_ROOT = Path(".artifacts/alpha_lab/batch3_b3f3_session_open_mr_v1")


class B3F3ScreenError(ValueError):
    """Raised on any violation of the frozen B3F3 screen contract."""
...
def _rows_from_trades(trades: Sequence[B3F3Trade]) -> tuple[_TradeRow, ...]:
    rows = [
        _TradeRow(
            exit_ts=datetime.fromtimestamp(trade.exit_ns / 1_000_000_000, tz=UTC),
            signal_ts=datetime.fromtimestamp(trade.entry_ns / 1_000_000_000, tz=UTC),
            pnl=trade.realized_pnl_usd,
            direction=trade.direction,
            exit_reason=trade.exit_reason,
            holding_seconds=(trade.exit_ns - trade.entry_ns) / 1_000_000_000,
            displacement_at_entry=trade.displacement_at_entry,
        )
        for trade in trades
    ]
...
def expectancy_and_profit_factor(pnls: Sequence[Decimal]) -> tuple[Decimal, Decimal]:
    if not pnls:
        raise B3F3ScreenError("expectancy/profit-factor requires at least one trade")
...
def best_5pct_removed_expectancy(rows: Sequence[_TradeRow]) -> Decimal:
    if not rows:
        raise B3F3ScreenError("best-5%% removal requires at least one trade")
...
def fold_positive_count(
    ...
    if len(fold_boundaries) < 2:
        raise B3F3ScreenError("fold_boundaries must contain at least 2 edges")
    positive = 0
...
def _displacement_stats(rows: Sequence[_TradeRow]) -> tuple[float | None, float | None]:
    if not rows:
        return None, None
    displacements = [row.displacement_at_entry for row in rows]
    return statistics.mean(displacements), statistics.median(displacements)


def _entry_minute_of_window_stats(
    rows: Sequence[_TradeRow],
) -> tuple[float | None, float | None]:
    """Mean/median minutes-past-08:05-London at entry -- a coarse,
    report-only "entry-time distribution within the window" summary."""
    ...
    minutes = []
    for row in rows:
        local = row.signal_ts.astimezone(london)
        minutes.append((local.hour * 60 + local.minute) - (8 * 60 + 5))
    return statistics.mean(minutes), statistics.median(minutes)


@dataclass(frozen=True, slots=True)
class B3F3ScorecardRow:
    instrument_id: str
    config: B3F3Config
    ...
    mean_displacement_at_entry: float | None
    median_displacement_at_entry: float | None
    mean_entry_minute_past_open: float | None
    median_entry_minute_past_open: float | None


def evaluate_b3f3_config(
    *,
    instrument_id: str,
    config: B3F3Config,
    native_trades: Sequence[B3F3Trade],
    stressed_1_5x_trades: Sequence[B3F3Trade],
    stressed_2_0x_trades: Sequence[B3F3Trade],
    ...
    mean_disp, median_disp = _displacement_stats(native_rows)
    mean_entry_minute, median_entry_minute = _entry_minute_of_window_stats(native_rows)
    ...
    return B3F3ScorecardRow(
        ...
        mean_displacement_at_entry=mean_disp,
        median_displacement_at_entry=median_disp,
        mean_entry_minute_past_open=mean_entry_minute,
        median_entry_minute_past_open=median_entry_minute,
    )


# ---------------------------------------------------------------------------
# Parameter-neighborhood robustness (3-D: displacement_threshold x
# stop_atr x target_mode; instrument identity never an adjacency dimension)
# ---------------------------------------------------------------------------


def _cell_for_config(config: B3F3Config) -> tuple[int, int, int]:
    return (
        DISPLACEMENT_THRESHOLD_GRID.index(config.displacement_threshold),
        STOP_ATR_GRID.index(config.stop_atr),
        TARGET_MODE_GRID.index(config.target_mode),
    )


def largest_connected_passing_region(rows: Sequence[B3F3ScorecardRow]) -> int:
    instrument_ids = {row.instrument_id for row in rows}
    if len(instrument_ids) > 1:
        raise B3F3ScreenError(
...
def sleeve_passes_robustness(rows: Sequence[B3F3ScorecardRow]) -> bool:
...
_SCORECARD_FIELDS = [
    field.name for field in B3F3ScorecardRow.__dataclass_fields__.values()
]


def _scorecard_row_to_dict(row: B3F3ScorecardRow) -> dict[str, Any]:
    payload = asdict(row)
    config = payload.pop("config")
    payload["displacement_threshold"] = str(config["displacement_threshold"])
    payload["stop_atr"] = str(config["stop_atr"])
...
def write_b3f3_artifacts(
    *,
    scorecard: Sequence[B3F3ScorecardRow],
    pair_robustness: Sequence[dict[str, Any]],
    metadata: dict[str, Any],
    output_dir: Path,
) -> None:
    if output_dir.exists():
        raise B3F3ScreenError(f"{output_dir} already exists; refusing to overwrite")
    output_dir.mkdir(parents=True)

    scorecard_path = output_dir / "scorecard.csv"
    with scorecard_path.open("w", newline="", encoding="utf-8") as handle:
        fieldnames = ["displacement_threshold", "stop_atr", "target_mode"] + [
