...
EVALUATOR_VERSION = "eurusd-session-range-expansion-v1-development-evaluator-1"
RESULT_SCHEMA = "ftmoquant.g1-eurusd-session-range-expansion-development-results"
BASE_RESEARCH_UNITS = Decimal("100000")
_EURUSD = "EUR/USD.DUKASCOPY"
_SESSION_ZONE = ZoneInfo("America/New_York")


class EurusdSessionRangeExpansionEvaluationError(ValueError):
    """Raised when the frozen production evaluation cannot be honored exactly."""


class PreparedEurusdSessionRangeExpansionMarketData:
...
class EurusdSessionRangeExpansionFamily(
    StrategyFamily[Sequence[tuple[int, int, Decimal]], tuple[Any, ...]]
):
    """The frozen exact 9-cell ``eurusd_session_range_expansion_v1`` family."""

    def __init__(self, spec: EurusdSessionRangeExpansionSpec | None = None) -> None:
        self.spec = load_eurusd_session_range_expansion_spec() if spec is None else spec
        self._parameter_space = ParameterSpace(
            (
                CategoricalParameter(
                    "breakout_window_end", self.spec.breakout_window_end_grid
                ),
                CategoricalParameter("scheduled_exit", self.spec.scheduled_exit_grid),
            )
        )
    ...
    def create_state(
        ...
        return EurusdSessionRangeExpansionState(_parameters(parameters))
    ...
    def neighbours(
        ...
        dimensions: tuple[tuple[str, tuple[ParameterValue, ...]], ...] = (
            ("breakout_window_end", self.spec.breakout_window_end_grid),
            ("scheduled_exit", self.spec.scheduled_exit_grid),
        )
...
class EurusdSessionRangeExpansionTrialEvaluator:
    ...
    def __init__(
        ...
        spec: EurusdSessionRangeExpansionSpec,
    ...
    def __call__(
        ...
        if not isinstance(family, EurusdSessionRangeExpansionFamily):
            raise EurusdSessionRangeExpansionEvaluationError(
...
def prepare_eurusd_session_range_expansion_market_data(
    ...
    _validate_catalog_trees(context, development_roots)
    return PreparedEurusdSessionRangeExpansionMarketData(
        instrument, minute, execution_frames, daily
    )


def run_eurusd_session_range_expansion_development(
    *,
    spec_path: Path,
    ...
    """Execute the exact 9-cell DEVELOPMENT grid and persist deterministic output."""

    if output_dir.exists():
        raise FileExistsError(f"output already exists: {output_dir}")
    _require_clean_worktree()
    spec = load_eurusd_session_range_expansion_spec(spec_path)
    if spec.semantic_sha256 != EURUSD_SESSION_RANGE_EXPANSION_SEMANTIC_SHA256:
        raise EurusdSessionRangeExpansionEvaluationError(
    ...
    prepared = prepare_eurusd_session_range_expansion_market_data(
        context, development_roots
    )
    ...
    if result.exact_trial_count != 9:
        raise EurusdSessionRangeExpansionEvaluationError(
            "production search did not attempt 9 cells"
        )
...
class _SessionRangeExpansionExecutor(Strategy):
    def __new__(
        cls,
    ...
    ) -> _SessionRangeExpansionExecutor:
        ...
        super().__init__(
            StrategyConfig(
                strategy_id=StrategyId(
                    "EURUSD-SESSION-RANGE-EXPANSION-V1-DEVELOPMENT-001"
                ),
                log_events=False,
                log_commands=False,
            )
    ...
    def on_bar(self, bar: Bar) -> None:
        ...
        if local.weekday() <= 4 and local.hour == 17 and local.minute == 0:
            self.clock.set_time_alert_ns(
                name=f"eurusd-session-range-mark-{bar.ts_event}",
...
def _run_native_fold(
    *,
    ...
    executor = _SessionRangeExpansionExecutor(
    ...
    try:
        ...
        engine.run(
            ...
            run_config_id=f"G1SRE-{identity[:20]}",
        )
...
def _parameters(
    ...
    breakout = parameters.get("breakout_window_end")
    exit_ = parameters.get("scheduled_exit")
    if not isinstance(breakout, str) or not isinstance(exit_, str):
    ...
    return EurusdSessionRangeExpansionParameters(
        breakout_window_end=_parse_hhmm(breakout),
        scheduled_exit=_parse_hhmm(exit_),
    )
...
def _validate_window(
    ...
    if window.fold_id not in expected or window != expected[window.fold_id]:
        raise EurusdSessionRangeExpansionEvaluationError("requested fold is not frozen")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Execute frozen eurusd_session_range_expansion_v1 exact DEVELOPMENT grid"
        )
    )
    parser.add_argument(
        "--spec", type=Path, default=EURUSD_SESSION_RANGE_EXPANSION_SPEC_PATH
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    _, _, summary = run_eurusd_session_range_expansion_development(
