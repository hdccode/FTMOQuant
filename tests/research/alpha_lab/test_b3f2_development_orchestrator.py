...
import ftmoquant.research.alpha_lab.b3f2_development_orchestrator as orchestrator
from ftmoquant.research.alpha_lab.b3f1_spread_signals import PAIR_UNIVERSE
from ftmoquant.research.alpha_lab.b3f2_asian_range_fade_signals import build_b3f2_grid
from ftmoquant.research.alpha_lab.b3f2_development_orchestrator import (
    FROZEN_PREREGISTRATION_SHA256,
    B3F2OrchestratorError,
    build_metadata,
...
def _synthetic_m1(instrument_id: str, root: Path, start_utc, end_exclusive_utc):
    n = 800  # spans well past the Asian range + entry window for day 1
...
def test_reject_forbidden_root_catches_every_forbidden_token(
    ...
    with pytest.raises(B3F2OrchestratorError, match="forbidden"):
        reject_forbidden_root(
...
def test_reserve_output_directory_rejects_existing_path(tmp_path: Path) -> None:
    ...
    existing.mkdir()
    with pytest.raises(B3F2OrchestratorError, match="already exists"):
...
def test_metadata_flags_session_definition_conflict() -> None:
    metadata = build_metadata(
        readiness_document={}, tested_instrument_count=7, tested_config_count=84
    )
    assert "conflict_with_existing_convention" in metadata["session_definitions"]
    assert (
        "g1.sessions"
        in metadata["session_definitions"]["conflict_with_existing_convention"]
    )
...
def test_no_redundant_m15_or_range_reconstruction_per_config(tmp_path: Path) -> None:
    from ftmoquant.research.alpha_lab import b3f2_development_orchestrator as mod

    spec = PAIR_UNIVERSE[0]
    with (
        patch.object(mod, "load_m1_bidask", side_effect=_synthetic_m1),
        patch.object(
            mod, "derive_m15_mid_ohlc", wraps=mod.derive_m15_mid_ohlc
        ) as m15_spy,
        patch.object(
            mod, "compute_asian_ranges", wraps=mod.compute_asian_ranges
        ) as ranges_spy,
        patch.object(
    ...
    assert len(rows) == 12
    assert m15_spy.call_count == 1
    assert ranges_spy.call_count == 1
...
def test_grid_size_is_84_across_7_instruments() -> None:
    ...
    assert len(build_b3f2_grid()) == 12
    assert len(PAIR_UNIVERSE) * len(build_b3f2_grid()) == 84
